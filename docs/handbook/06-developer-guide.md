...
metaphor migration generate Invoice performance
metaphor migration run
...
(`performance` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed performance          # run Rust seeders
metaphor migration generate-seeds performance  # emit SQL seed files
```
