//! Application service for Reward entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `reward_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateRewardDto, UpdateRewardDto};
use crate::domain::entity::Reward;
use crate::infrastructure::persistence::RewardRepository;

/// Application service for Reward entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `reward_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct RewardServiceCustom {
///     inner: Arc<RewardService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type RewardService = GenericCrudService<
    Reward,
    CreateRewardDto,
    UpdateRewardDto,
    RewardRepository,
>;

// <<< CUSTOM
// END CUSTOM
